...
export const FormInput: FC<FormInputProps> = ({ label, ...otherProperties }) => {
  return (
    <div className="relative">
      ...
      <label
        className={`
          text-gray-400 font-bold tracking-wide absolute left-[20px] top-[15px] pointer-events-none transition-all
          peer-focus:top-[-10px] peer-focus:left-[-3px] peer-focus:text-black peer-focus:text-sm
          ${otherProperties.value && typeof otherProperties.value === "string" && otherProperties.value.length > 0 ? "absolute top-[-5px] left-[-5px] text-black text-sm pointer-events-auto" : ""}
        `}
      >
        {label}
      </label>
